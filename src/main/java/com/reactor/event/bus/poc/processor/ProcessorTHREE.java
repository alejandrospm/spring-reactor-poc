...
@Slf4j
@Service
public class ProcessorTHREE extends BaseSubscriber<Target> {
    ...
    @Autowired
    public ProcessorTHREE(final ReplayProcessor replayProcessor){
    ...
    protected void hookOnNext(final Target target) {
        ...
        if(target.getStatus().equals(Status.STEP_3)) {
            log.info("Event name:" + target.getName());
            ProcessorUtil.processTime();
            target.setStatus(Status.STEP_4);
